...
abstract class Repository<Item> {
  ...
  Stream<List<Item>> fetchMultiple(Iterable<Id<Item>> ids) {
    ...
    final sendSnapshot = () async {
      var snapshot = await Future.wait(allStreams.map((stream) => stream.first));
      controller.add(snapshot);
    };

    controller = StreamController<List<Item>>(
      onListen: () => allStreams.forEach((stream) => stream.listen((_) => sendSnapshot())),
  ...
  Stream<Map<Id<Item>, Item>> fetchAll() {
    if (isFinite) {
      throw UnimplementedError("A fetchAll method was called on repository $this. It's finite, so "
          "that should be possible. Make sure you implement the fetchAll "
          "method of ${this.runtimeType} and don't call super.fetchAll.");
    } else {
      throw UnsupportedError("fetchAll was called on repository $this, altough it's not finite. "
          "Don't do that.");
    }
  }

  /// Fetches all ids. May only be called if this [isFinite].
  Stream<List<Id<Item>>> fetchAllIds() => fetchAll().map((all) => all.keys.toList());

  /// Fetches all items. May only be called if this [isFinite].
  Stream<List<Item>> fetchAllItems() => fetchAll().map((all) => all.values.toList());

  /// Updates an item. May only be called if this [isMutable].
  Future<void> update(Id<Item> id, Item item) {
    if (isMutable) {
      throw UnimplementedError("update was called on repository $this. It's mutable, so that should "
          "be possible. Make sure you implement the update method of "
          "${this.runtimeType} and don't call super.update.");
    } else {
      throw UnsupportedError("update was called on repository $this, altough it's not mutable. "
          "Don't do that.");
    }
  }
  ...
  Future<void> remove(Id<Item> id) {
    if (isMutable) {
      throw UnimplementedError("remove was called on repository $this. It's mutable, so that should "
          "be possible. Make sure you implement the remove method of "
          "${this.runtimeType} and don't call super.remove.");
    } else {
      throw UnsupportedError("remove was called on repository $this, altough it's not mutable. "
          "Don't do that.");
    }
  }

  /// Clears all the items. May only be called if this [isMutable] and [isFinite].
  Future<void> clear() => fetchAllIds().first.then((ids) => ids.forEach((id) => remove(id)));
